    ...
    for k, v in ipairs(cinemaLocations) do
        ...
        if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z) < 4.8 then
            ...
            if IsControlPressed(0, 38) then
                ...
                helpDisplay("press ~r~ESC ~w~key to exit", 0)
            end
        end
    end
end
...
Citizen.CreateThread(function()
    if GetRoomKeyFromEntity(PlayerPedId()) ~= -1337806789 and
        DoesEntityExist(GetClosestObjectOfType(319.884, 262.103, 82.917, 20.475, cin_screen, 0, 0, 0)) then

        DeconstructMovie()
    end
    if GetRoomKeyFromEntity(PlayerPedId()) ~= 1196036993 and
        DoesEntityExist(GetClosestObjectOfType(-802.0, 343.19, 158.81, cin_screen, 0, 0, 0)) then
    end
    -- Create the cinema's
    Load()
end)
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 322) and GetRoomKeyFromEntity(PlayerPedId()) == -1337806789 then
            DoScreenFadeOut(1000)
            SetEntityCoords(playerPed, ReturnPlayerCoords.x, ReturnPlayerCoords.y, ReturnPlayerCoords.z, true, true,
                true, true)
            Citizen.Wait(30)
            DoScreenFadeIn(800)
